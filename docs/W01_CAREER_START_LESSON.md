...
교수자 메시지:
> 이름과 학번은 입력하지 않습니다. 대신 지금 만드는 Jobfit 참여코드를 한 학기 동안 계속 사용합니다.
...
- 다른 사람의 실명·연락처 등 제3자 개인정보를 입력하지 않는다.
- 건강·가족사 등 진로활동에 필요하지 않은 민감정보를 입력할 필요가 없다.
...
백업 보안 안내:
- 개인 진로·취업자료가 포함될 수 있으므로 공용PC·공유폴더에 저장하지 않는다.
- 공용기기를 사용했다면 수업 후 브라우저 저장자료와 다운로드 파일을 확인해 삭제한다.
