...
5. 고용24 대학생진로준비도검사 PRE 결과를 입력한다.
...
교수자 메시지:
> 이름과 학번은 입력하지 않습니다. 대신 지금 만드는 Jobfit 참여코드를 한 학기 동안 계속 사용합니다. 이 코드는 같은 학생의 학습기록과 PRE·POST를 연결하기 위한 가명화된 참여자 코드이며, 완전 익명 식별자로 표현하지 않습니다.

학생은 Jobfit 참여코드를 생성하고 코드 복사 또는 메모를 한다.
...
개인정보 안내:
- 이름·학번·전화번호·이메일·주소를 외부 AI에 입력하지 않는다.
- 제3자의 실명이나 개인 식별정보를 입력하지 않는다.
- 진로상담에 필요하지 않은 건강·가족사 등 민감정보를 입력할 필요가 없다.
- AI가 정리한 내용은 학생이 사실 여부를 확인한 뒤 저장한다.
...
## 60–83분 | Work24 PRE · 대학생진로준비도검사
학생은 고용24에서 `대학생진로준비도검사`를 실시한다.

Jobfit에는 결과표의 다음 14개 하위요인 T점수를 그대로 입력한다.
1. 계획성
2. 독립성
3. 자신지식(자기이해)
4. 진로활동경험
...
운영원칙:
- `대학생진로준비도검사`라는 검사명을 반드시 확인한다.
- S형/L형, 대학생 진로준비도검사와 혼동하지 않는다.
- 검사일과 14개 하위요인 T점수만 Jobfit에 기록한다.
...
현재 Jobfit에는 논문 부록의 한국판 최종 문항과 버전 정보가 LOCK되어 있다.
다만 타당화 논문에 문항이 공개되어 있다는 사실과 웹 프로그램에서 재배포할 수 있는 권리는 동일하지 않다. 연구 수집을 활성화하기 전 저자·발행기관·원척도 권리조건을 별도로 확인하고 그 결과를 MEASURE_PROVENANCE.md에 기록한다.
...
학생에게 안내:
- 백업파일에는 이후 개인 경험·취업준비 내용이 포함될 수 있으므로 공용PC·공유폴더에 저장하지 않는다.
- 공용기기를 사용했다면 수업 종료 후 다운로드 파일과 브라우저 저장정보를 정리한다.
- 다운로드 폴더에만 두지 않는 것을 권장한다.
- Jobfit 참여코드를 별도 메모해도 좋다.
...
Jobfit 사용·출석·성적과 연구 참여는 분리하며, 연구 미참여 또는 철회로 수업 이용·평가·피드백에서 불이익이 없어야 한다. 연구를 활성화할 때에는 별도 동의화면에서 보존기간, 폐기방법, 철회 절차와 철회 이후 자료 처리, 제3자 제공 여부, 연구자 연락처를 안내한다.
...
- Jobfit 참여코드 생성·복사 확인
- Career Start 저장 확인
- 고용24 대학생진로준비도검사 링크 확인
- PRE 고용24 14개 T점수 입력 확인
