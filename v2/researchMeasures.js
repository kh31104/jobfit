...
export function renderMeasurePanel(ctx,timepoint='pre'){
  ...
  return `<div class="hr"></div><div class="block researchMeasurePanel" data-timepoint="${prefix}">
    ...
    <div class="summaryBox">
      <div class="sectionHead"><div><div class="kicker">PRIMARY OUTCOME · WORK24</div><h3>고용24 구직준비도검사</h3><p class="help">현재 구직준비 상태를 확인하는 고용24 공식 검사입니다. 고용24에서 검사한 뒤 결과표에 표시된 9개 결과점수를 이 화면에 그대로 입력합니다.</p></div><span class="badge">대학생·성인 · 공식검사</span></div>
      <div class="actions"><a class="btn secondary" href="${WORK24_URL}" target="_blank" rel="noopener">구직준비도검사 화면 열기 ↗</a><a class="btn outline" href="https://www.work24.go.kr/wk/r/c/1000/jobPsyExamRsltList.do" target="_blank" rel="noopener">내 검사 결과 확인 ↗</a></div>
      <div class="callout warn"><b>고용24에서 ‘구직준비도검사’를 선택하세요.</b><br>필터에서 <b>대학생 → 진로(취업)준비도</b>를 선택하면 찾기 쉽습니다. 구직준비도검사·찾아Dream과 다른 검사이며 PRE와 POST에서 동일한 검사를 사용합니다.</div>
      ...
      <div class="status" id="${prefix}Work24Status">${complete(w.scores,14)?'9개 결과점수 입력 완료':''}</div>
    </div>
...
export function bindMeasurePanel(ctx,timepoint='pre'){
  ...
  saveBtn.addEventListener('click',()=>{
    ...
    const block={
      ...
      work24JobReadiness:{instrument:'고용24 구직준비도검사',source:'Work24/Korea Employment Information Service',examDate:document.getElementById(`${prefix}Work24Date`)?.value||'',itemCount:9,scores:w,labels:WORK24_LABELS,scoreType:'official-result-score',scoreSchema:'official-result-subscales-9',wordingStatus:'official-external-test-results-only'},
      kcaas:kcaasBlock(k),
    ...
    const missing=[];if(!complete(w,9))missing.push('고용24 9개 결과점수');if(!complete(k,KCAAS_SCALE.itemCount))missing.push('진로적응성 12문항');if(uInputs.length&&!complete(u,SUDCO_SCALE.itemCount))missing.push('강점활용·약점교정 9문항');
