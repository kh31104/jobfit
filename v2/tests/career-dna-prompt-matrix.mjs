...
await scenario('Partial VIA and MI are explicitly marked',async page=>{await expandModule(page,'VIA 성격강점');await page.locator('#via_0').fill('학구열');await page.locator('#via_1').fill('신중성');await expandModule(page,'다중지능검사');await page.locator('#mi_0').selectOption({label:'언어지능'})},async({prompt})=>{
...
await scenario('Complete Career Anchor and all modules are integrated with SWOT',async page=>{
  ...
  await expandModule(page,'내가 생각하는 나의 강점');for(const i of [0,1,2,3,4])await page.locator('[data-strength]').nth(i).click();await expandModule(page,'VIA 성격강점');for(const [i,v] of ['학구열','신중성','진실성','희망','친절'].entries())await page.locator(`#via_${i}`).fill(v);await expandModule(page,'다중지능검사');for(const [i,v] of ['자기성찰지능','언어지능','인간친화지능'].entries())await page.locator(`#mi_${i}`).selectOption({label:v});await expandModule(page,'내가 생각하는 나 × 검사에서 나타난 나');await page.locator('#compare_unexpected').fill('인간친화지능은 예상과 달랐다.');
},async({prompt})=>{
