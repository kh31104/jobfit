...
await scenario('No completed module does not invent data',async()=>{},async({prompt})=>{
  assert(!prompt.includes('[Balance Game'),'Empty balance should be omitted');assert(!prompt.includes('[Career Anchor · 검사결과]'),'Empty anchor should be omitted');assert(!prompt.includes('[VIA 성격강점]'),'Empty VIA should be omitted');assert(!prompt.includes('[다중지능]'),'Removed MI module must stay omitted');assert(prompt.includes('입력되지 않은 내용은 추정하지 않는다.'),'No-inference guard missing');assert(prompt.includes('O와 T의 근거가 없으면'),'SWOT opportunity/threat no-inference guard missing');
});

await scenario('Partial VIA is explicitly marked',async page=>{await expandModule(page,'VIA 성격강점');await page.locator('#via_0').fill('학구열');await page.locator('#via_1').fill('신중성')},async({prompt})=>{
  assert(prompt.includes('[VIA 성격강점]'),'VIA module missing');assert(prompt.includes('VIA가 2/5만 입력됨'),'Partial VIA warning missing');assert(!prompt.includes('[다중지능]'),'Removed MI module leaked into prompt');
});
