...
async function runSavedBrowser(){
  ...
  try{
    ...
    assert(persisted?.assessments?.careerDNA?.selfStrengths?.[0]==='학습','STEP 1 self-strength selection was lost after same-browser reload');
    ...
    assert(persisted?.assessments?.careerDNA?.selfStrengths?.[0]==='학습','IndexedDB recovery lost STEP 1 self-strength selection');
