...
await run('Week 3 renders the agreed Career DNA standard sequence collapsed by title',async page=>{
  ...
  assert(!body.includes('다중지능검사'),'Multiple-intelligence module must not remain in Week 3');
...
await run('Qualitative and quantitative inputs produce SWOT integration prompt and preserve STEP2 bridge',async page=>{
  ...
  assert(prompt.includes('[내가 생각하는 나의 강점]'),'Self-strength module missing from prompt');assert(prompt.includes('[VIA 성격강점]'),'VIA missing from prompt');assert(!prompt.includes('[다중지능]'),'Removed MI module leaked into prompt');
