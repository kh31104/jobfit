...
export async function render(ctx){
  ...
  const dnaReflection=state.assessments?.careerDNA?.reflection||{};
  ...
  root.innerHTML=`<section class="card">
    ...
    ${reflectionBridge(dnaReflection,ctx)}
  ...
  function makePrompt(){
    const ref=dnaReflection||{};
    const refLines=[];
    if(ref.fit)refLines.push(`일치한다고 느낀 결과: ${ref.fit}`);
    if(ref.question)refLines.push(`확인이 필요한 결과: ${ref.question}`);
    if(ref.disagree)refLines.push(`동의하기 어려운 결과: ${ref.disagree}`);
    const refBlock=refLines.length?`\n\n[STEP 1에서 내가 직접 적은 확인 메모 · 참고만]\n${refLines.join('\n')}`:'';
    return `지금부터 내 경험을 분석하는 취업 면접관이 되어줘. 목표는 자소서를 대신 쓰는 것이 아니라, 내가 실제로 한 행동과 증거를 정확히 꺼내는 것이다.\n\n[경험 기본정보]\n유형: ${v('category')||'미입력'}\n경험: ${v('title')||'미입력'}\n기간: ${v('period')||'미입력'}\n진행방식: ${v('workMode')||'미입력'}\n내 역할: ${v('roleTitle')||v('role')||'미입력'}\n배경: ${v('context')||'미입력'}${refBlock}\n\n[인터뷰 규칙]\n1. 반드시 한 번에 질문 하나만 한다. 전체 질문은 최대 10개다.\n2. 내가 '우리 팀이', '저희가'라고 답하면 바로 '그중 본인이 직접 한 행동은 무엇인가요?'라고 다시 묻는다.\n3. Challenge → Action → Reasoning → Result → Evidence → Learning 순으로 파고든다.\n4. Action에서는 실제 행동동사, 사용한 도구·자료, 누구와 어떻게 상호작용했는지 묻는다.\n5. Result에서 숫자가 없으면 숫자를 만들도록 유도하지 않는다. 대신 산출물, 완료여부, 피드백, 변화, 기록 등 실제 확인 가능한 결과를 묻는다.\n6. 내가 말하지 않은 숫자·성과·역할·도구·고객반응은 절대 만들어내지 않는다.\n7. 모호한 역량명부터 제시하지 말고 행동 사실을 먼저 충분히 확보한다.\n8. STEP 1 확인 메모가 있더라도 경험을 검사결과에 끼워 맞추지 않는다. 이 경험이 실제로 관련될 때만 그 해석을 확인하거나 반박할 수 있는 근거가 있는지 묻는다.\n9. 인터뷰가 끝나면 내가 말한 내용만으로 Situation / Role / Challenge / Action / Reasoning / Result / Evidence / Learning을 정리한다.\n10. 역량은 최대 5개만 제안하고, 각 역량 옆에 '근거 행동 한 문장'을 붙인다. 근거가 약하면 '근거 약함'이라고 표시한다. 마지막에 확인이 필요한 사실이나 과장 위험이 있는 문장을 별도로 표시한다.\n\n첫 질문부터 시작해줘.`
  }
...
function reflectionBridge(ref,ctx){const items=[['일치한다고 느낀 결과',ref?.fit],['확인이 필요한 결과',ref?.question],['동의하기 어려운 결과',ref?.disagree]].filter(([,v])=>v);if(!items.length)return '<div class="callout good"><b>STEP 1 → STEP 2</b> · Career DNA 점수는 경험의 정답으로 가져오지 않습니다. 지금부터 실제 경험을 독립적으로 확인합니다.</div>';return `<div class="callout good"><b>STEP 1 → STEP 2 · 경험으로 검증하기</b><br>아래 내용은 검사점수가 아니라 <b>내가 STEP 1에서 직접 적은 확인 메모</b>입니다. 경험을 끼워 맞추지 말고 실제 행동이 이를 확인하거나 반박하는지 살펴보세요.<div class="pillRow" style="margin-top:8px">${items.map(([k,v])=>`<span class="pill"><b>${ctx.escapeHtml(k)}</b> · ${ctx.escapeHtml(v)}</span>`).join('')}</div></div>`}
