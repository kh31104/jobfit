...
export async function render(ctx){
  ...
  root.innerHTML=`<section class="card experienceCompetencyWeek4">
    ${styleBlock()}
    <div class="sectionHead"><div><div class="kicker">STEP 2 · EXPERIENCE & COMPETENCY</div><h2>나의 경험에서 역량 후보 찾기</h2><p>3주차 Career DNA를 정답으로 확정하지 않고, <b>내가 실제로 한 행동</b>에서 강점과 역량 후보의 근거를 찾습니다. 직무 관련성은 이후 실제 직무정보와 대조해 확인합니다.</p></div><span class="badge">4주차</span></div>
    <div class="progress"><span style="width:21%"></span></div>
    <div class="callout info"><b>오늘의 흐름</b> · Career DNA 간단히 확인 → My Best 3 Experience → 대표 경험 선택 → AI Experience Interview → 행동·결과·증거 정리 → 강점·역량 후보 → Experience Map</div>
    <div class="callout good"><b>4주차의 도착점</b> · 직업을 정하는 시간이 아닙니다. <b>어떤 경험에서 내가 무엇을 했고, 그 행동이 어떤 강점·역량 후보와 연결되는지</b> 근거와 함께 정리합니다.</div>
    ...
    <div class="hr"></div><div class="block"><div class="moduleHead"><span>03</span><div><h3>AI Experience Interview</h3><p>AI가 답을 쓰는 것이 아니라, 경험 속 <b>내 행동·판단·결과·증거</b>를 한 질문씩 구체화하도록 사용합니다.</p></div></div>
      ...
      <div class="grid3" style="margin-top:12px">${sel('workMode','진행 방식','',['개인','팀','조직/부서'],ctx)}${score('contribution','내 기여도 자기평가')}${txt('roleTitle','내 역할 한 줄','','예: 자료분석 / 고객응대 / 일정조율',ctx)}</div>
      ...
      <div class="callout warn" style="margin-top:12px"><b>외부 AI 사용 전 개인정보 확인</b> · 이름·학번·전화번호·이메일을 넣지 말고, 팀원·고객·교수·상사의 실명은 ‘팀원 A’, ‘고객’, ‘교수자’처럼 역할명으로 바꾸세요. 회사 내부 비공개 수치·자료·영업비밀도 입력하지 마세요.</div><div class="actions" style="margin-top:12px"><button class="btn secondary" id="makeInterviewPrompt">AI 경험 인터뷰 프롬프트 만들기</button><button class="btn outline hidden" id="copyInterviewPrompt">프롬프트 복사</button></div><div class="promptBox hidden" id="interviewPrompt"></div>
    </div>
    ...
    <div class="hr"></div><div class="block"><div class="moduleHead"><span>04</span><div><h3>행동 → 판단 → 결과 → 증거</h3><p>AI와 대화한 뒤 확인된 사실만 내 경험카드에 정리합니다.</p></div></div>
      ...
      <div class="grid3" style="margin-top:12px">${sel('evidenceType','가장 강한 증거 유형','',EVIDENCE_TYPES,ctx)}${sel('evidenceGrade','근거 확인 수준 · 학생 자기평가','',['A · 객관적 자료로 확인 가능','B · 타인의 피드백/평가로 확인','C · 본인 설명 중심'],ctx)}${txt('actionVerbs','핵심 행동동사','','예: 비교했다, 분석했다, 조율했다',ctx)}</div>
      <div class="callout info" style="margin-top:10px"><b>A/B/C는 교육용 자기평가 기준입니다.</b> 표준화된 심리척도나 객관적 검증등급이 아니며, 이후 실제 자료와 직무정보를 확인하면서 수정할 수 있습니다.</div><div class="field" style="margin-top:12px"><label>내 원래 말 · Raw Voice</label><textarea id="rawVoice" placeholder="AI가 다듬기 전 내가 실제로 설명한 문장이나 메모"></textarea><span class="hint">나중에 자기소개서·면접답변을 내 말로 복원할 때 사용합니다.</span></div>
    ...
    <div class="hr"></div><div class="block"><div class="moduleHead"><span>06</span><div><h3>Experience Map</h3><p>저장한 경험들을 한눈에 보고, 어떤 역량 후보에 행동근거가 있는지 확인합니다.</p></div></div>
      <div id="experienceMapPreview">${experienceMapHtml(experiences,ctx)}</div>
      <div class="callout warn" style="margin-top:12px"><b>다음 연결</b> · 6주차에는 여기에서 만든 강점·역량 후보와 행동근거를 가지고 직무 후보를 탐색하고, 실제 직무의 Task·KSA·KPI와 매칭합니다.</div>
    </div>
...
function experienceMapHtml(items,ctx){
  ...
  return `<div class="experienceMapGrid">${items.map((x,i)=>`<div class="mapCard"><span class="rankTag">Experience ${i+1}</span><h4>${ctx.escapeHtml(x.title)}</h4><div><small>내 행동</small><p>${ctx.escapeHtml(x.action||'아직 정리하지 않음')}</p></div><div><small>결과·증거</small><p>${ctx.escapeHtml(x.result||'결과 미입력')}${x.evidence?` · ${ctx.escapeHtml(x.evidence)}`:''}</p></div><div><small>행동근거가 있는 역량 후보</small>${(x.competencyEvidence||[]).length?`<ul>${x.competencyEvidence.map(c=>`<li><b>${ctx.escapeHtml(c.keyword||'역량 후보')}</b> · ${ctx.escapeHtml(c.evidence||'근거행동 미입력')}</li>`).join('')}</ul>`:`<p>아직 행동근거가 붙은 역량 후보가 없습니다.</p>`}${(x.unverifiedCompetencyCandidates||[]).length?`<small>근거 미확인 추가 후보</small><p>${x.unverifiedCompetencyCandidates.map(c=>ctx.escapeHtml(c)).join(', ')}</p>`:''}</div></div>`).join('')}</div>`;
}
