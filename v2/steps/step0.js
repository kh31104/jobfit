...
export async function render(ctx){
  ...
  root.innerHTML=`<section class="card">
    ...
    <div class="hr"></div><div class="block"><div class="stepLabel">④</div><h3>현재 진로·취업 준비상태</h3><p class="help">잘 보이기 위해 높게 체크할 필요가 없습니다. 오늘의 상태가 학기 말 비교의 출발점입니다.</p>
      ...
      <div class="field" style="margin-top:12px"><label>My AI Career Rule · AI 활용 원칙</label><p class="help">직접 문장을 만들기 어렵다면 아래 추천문장을 넣으세요. AI 인터뷰에서 더 알맞은 원칙을 제안받았다면 그 문장을 그대로 붙여넣어도 됩니다.</p><textarea id="aiRule" placeholder="추천문장 버튼을 누르거나 AI가 제안한 활용 원칙을 붙여넣으세요.">${ctx.escapeHtml(b.aiRule||'')}</textarea><div class="actions"><button class="btn outline smallBtn" type="button" id="fillAiRule">추천문장 넣기</button></div></div>
    </div>
    ...
    <div class="hr"></div><div class="block"><div class="stepLabel">⑤</div><h3>AI LAB 00 · Career Check-in</h3><p class="help">AI에게 진로를 정해달라고 하지 않습니다. <b>AI가 나에게 질문하게</b> 만들어 지금의 상태를 말로 확인합니다.</p>
      ...
      <div class="callout info" style="margin-top:14px"><b>사용방법</b><br>① 프롬프트를 복사해 ChatGPT 등 AI에 붙여넣기 → ② AI의 질문에 최대 5번 답하기 → ③ AI가 마지막에 정리한 결과 전체를 아래 칸에 그대로 붙여넣기 → ④ ‘핵심내용 자동 정리’를 누르기</div>
  ...
  document.getElementById('fillAiRule').addEventListener('click',()=>{document.getElementById('aiRule').value='AI는 정보 정리와 질문 생성에 활용하고, 경험의 사실 여부와 진로에 대한 최종 판단은 내가 직접 확인한다.';ctx.toast('추천문장을 넣었습니다.');});
  ...
  document.getElementById('parseCheckinResult').addEventListener('click',()=>{const raw=v('careerCheckinResult'),parts=parseCheckinResult(raw),status=document.getElementById('checkinParseStatus');if(!raw){status.textContent='먼저 AI의 최종 결과를 붙여넣으세요.';return}if(parts.statement)document.getElementById('careerStartStatement').value=parts.statement;if(parts.action)document.getElementById('careerStartAction').value=parts.action;if(parts.aiRule)document.getElementById('aiRule').value=parts.aiRule;status.textContent=parts.statement&&parts.action?'출발점·실행행동·AI 활용 원칙을 자동으로 정리했습니다. 내용이 사실과 맞는지 확인하세요.':'일부 항목을 찾지 못했습니다. 아래 두 칸에 필요한 내용을 직접 복사해 주세요.';ctx.toast('AI 결과를 정리했습니다.');});
  ...
  function parseCheckinResult(raw){
    ...
    return {statement:pick(1,2),assets:pick(2,3),gap:pick(3,4),action:pick(4,5),aiRule:pick(5,6)};
  }
