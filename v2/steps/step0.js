export async function render(ctx){
  ...
  <section class="card">
    ...
    <div class="block"><h3>1. 이용 방식</h3><p class="help">전체 로드맵은 자기이해부터 Job Portfolio까지 모두 수행하고, 선택형은 필요한 도구만 이용합니다.</p>
      <div class="modeGrid"><div class="modeCard ${s.mode==='full'?'on':''}" data-mode="full"><b>전체 Career Roadmap</b><span>STEP 0–13 전체 과정을 누적합니다.</span></div><div class="modeCard ${s.mode==='selective'?'on':''}" data-mode="selective"><b>선택형 Career Tools</b><span>지금 필요한 검사·직무·지원서·면접 도구만 사용합니다.</span></div></div>
      ${c.lockMode?`<div class="callout info">이 수업에서는 교수자 설정에 따라 <b>${s.mode==='full'?'전체 Career Roadmap':'선택형 Career Tools'}</b>으로 고정되어 있습니다.</div>`:''}
    </div>

    <div class="hr"></div><div class="block"><h3>2. 수업 연결 · 익명코드</h3>
      <div class="grid2"><div class="field"><label>수업코드 <span class="muted">(선택)</span></label><input class="input" id="courseCode" value="${ctx.escapeHtml(p.courseCode||'')}" placeholder="교수자에게 받은 코드"></div><div class="field"><label>학교/기관 <span class="muted">(선택)</span></label><input class="input" id="institution" value="${ctx.escapeHtml(p.institution||'')}" placeholder="예: ○○대학교"></div></div>
      <div class="codeBox" style="margin-top:12px"><div><div class="muted" style="font-size:11px;font-weight:800">MY JOBFIT CODE</div><div class="anonCode" id="anonCode">${ctx.escapeHtml(p.anonCode||'아직 생성되지 않음')}</div></div><button class="btn outline" id="makeCodeBtn">${p.anonCode?'새 코드 발급':'익명코드 생성'}</button></div>
    ...
    <div class="hr"></div><div class="block"><h3>3. 기본 프로필</h3><p class="help">진로분석에 필요한 최소 배경정보입니다. 이름·학번·전화번호는 입력하지 않습니다.</p>
      <div class="grid3">
        ...
        ${sel('graduationPlan','졸업예정',p.graduationPlan,['','1년 이내','1–2년','2년 이상','졸업·수료','미정'])}
      </div>
    </div>
  ...
  document.getElementById('nextStep').addEventListener('click',()=>{save();ctx.navigate(1)});
