...
export async function render(ctx){
  ...
  root.innerHTML=`<section class="card">
    ...
    <div class="callout good"><b>오늘 할 일은 7개뿐입니다.</b><br>① 수업 연결 확인 → ② Jobfit 참여코드 → ③ 기본정보 → ④ 현재 준비상태 → ⑤ AI Career Check-in → ⑥ PRE 측정 → ⑦ 백업 저장</div>
    <div class="journeyStrip">${['수업 연결','Jobfit 참여코드','기본정보','현재 준비상태','AI Check-in','PRE 측정','백업'].map((x,i)=>`<span><b>${i+1}</b>${x}</span>`).join('')}</div>
    <div class="callout info" data-no-research-notice="1"><b>수업 데이터 저장 안내</b><br>수업 운영을 위해 Jobfit 참여코드, STEP 진행상태와 일부 구조화된 활동 결과가 중앙 서버에 저장됩니다.<br>이름·학번·연락처와 경험 서술, AI 대화, 이력서·자기소개서·면접답변 원문은 중앙 서버에 저장하지 않습니다.<br>향후 교육·진로 관련 연구에 활용하려는 경우에는 필요한 IRB 절차와 별도 연구참여 동의를 거친 자료만 사용하며, 연구 참여 여부는 수업 참여와 성적에 영향을 주지 않습니다.</div>
    ...
    <div class="hr"></div><div class="block"><div class="stepLabel">④</div><h3>현재 진로·취업 준비상태</h3><p class="help">잘 보이기 위해 높게 체크할 필요가 없습니다. 오늘의 상태가 학기 말 비교의 출발점입니다.</p>
      ...
      <div class="callout info" style="margin-top:12px"><b>Jobfit 기본 AI 활용 원칙</b><br>${DEFAULT_AI_CAREER_RULE}<br><span class="muted">학생이 별도로 작성하지 않아도 ‘Career Start 저장’을 누르면 자동 저장됩니다.</span></div>
    </div>
    ...
    <div class="hr"></div><div class="block"><div class="stepLabel">⑤</div><h3>AI LAB 00 · Career Check-in</h3><p class="help">AI에게 진로를 정해달라고 하지 않습니다. <b>AI가 나에게 질문하게</b> 만들어 지금의 상태를 말로 확인합니다.</p>
      ...
      <div class="callout info" style="margin-top:14px"><b>사용방법</b><br>① 프롬프트를 복사해 ChatGPT 등 AI에 붙여넣기 → ② AI의 질문에 최대 4번 답하기 → ③ AI가 마지막에 정리한 결과 전체를 아래 칸에 그대로 붙여넣기 → ④ ‘핵심내용 자동 정리’를 누르기<br><span class="muted">외부 AI에는 이름·학번·전화번호·이메일·주소 등 개인 식별정보와 다른 사람의 실명을 입력하지 마세요. 진로상담에 필요하지 않은 건강·가족사 등 민감정보도 입력할 필요가 없습니다.</span></div>
    ...
    <div class="hr"></div><div class="block finishBlock"><div class="stepLabel">⑦</div><h3>저장·백업 안내</h3><p class="help">학생의 입력 내용은 먼저 <b>현재 사용 중인 브라우저</b>에 저장됩니다. Jobfit 참여코드 기반 일부 구조화 결과는 수업 운영용 중앙 DB에도 자동 저장됩니다.</p>
      <div class="callout info"><b>학생과 교수자가 하는 일</b><br><b>학생 필수:</b> ‘Career Start 저장’ → ‘내 학습 백업파일 다운로드’ → 본인 이메일·카카오톡·클라우드에 보관<br><b>교수자:</b> 수업 운영을 위해 Jobfit 참여코드별 진행상태와 일부 구조화 결과를 확인할 수 있습니다. 경험 서술, AI 대화, 지원서·면접답변 원문은 중앙 운영 DB에 저장하지 않습니다.</div>
      <div class="callout good"><b>내 학습 백업파일에 저장되는 정보</b><br>Jobfit 참여코드, 기본정보, 현재 준비상태, 검사 입력값, STEP별 선택·작성 내용과 AI 결과 붙여넣기 내용이 저장됩니다. 이 파일은 학생 본인의 학습을 이어가기 위한 개인 백업이며, 이름·학번·전화번호는 입력하거나 저장하지 않습니다.</div>
  ...
  document.getElementById('makeCodeBtn').addEventListener('click',()=>{
    ...
    if(existing.startsWith('JF26-')){
      ...
      ctx.toast('기존 Jobfit 참여코드는 이번 학기 동안 유지됩니다.');
    }else ctx.toast('Jobfit 참여코드를 발급했습니다. 백업파일에도 함께 저장됩니다.');
  });
  document.getElementById('copyCodeBtn').addEventListener('click',async()=>{const code=document.getElementById('anonCode').textContent;if(!code.startsWith('JF26-')){ctx.toast('Jobfit 참여코드를 먼저 생성해 주세요.');return}try{await navigator.clipboard.writeText(code);ctx.toast('Jobfit 참여코드를 복사했습니다.')}catch{ctx.toast('코드를 길게 눌러 직접 복사해 주세요.')}});
  ...
  document.getElementById('withdrawResearchBtn')?.addEventListener('click',async()=>{
    if(!confirm('연구 참여를 철회하고 중앙 연구 DB에 제출된 연구자료를 삭제할까요? 수업용 학습데이터와 Jobfit 이용에는 영향을 주지 않습니다.'))return;
    const button=document.getElementById('withdrawResearchBtn'),statusEl=document.getElementById('centralResearchStatus');
    try{button.disabled=true;statusEl.textContent='연구 참여 철회와 연구자료 삭제를 처리하고 있습니다.';const result=await ctx.withdrawResearchData();statusEl.textContent=`연구 참여 철회 완료 · ${formatTime(result.withdrawn_at||new Date().toISOString())} · 수업용 학습데이터는 유지`;statusEl.className='status goodText';button.remove();ctx.toast('연구 참여 철회와 연구자료 삭제가 완료되었습니다.')}catch(err){statusEl.textContent=err.message||'연구 참여 철회에 실패했습니다.';statusEl.className='status errorText';button.disabled=false;ctx.toast('연구 참여 철회 상태를 확인해 주세요.')}
  });
  ...
  function save(show=true){
    ...
    if(show){const missing=[];if(!current.profile?.anonCode&&!document.getElementById('anonCode').textContent.startsWith('JF26-'))missing.push('Jobfit 참여코드');if(!profile.age||!profile.grade)missing.push('기본정보');if(!baseline.jobDecision||!baseline.prepStage)missing.push('준비상태');document.getElementById('status').textContent=missing.length?`저장했습니다. 아직 확인할 항목: ${missing.join(' · ')}`:'Career Start가 저장되었습니다. PRE 측정과 백업 보관까지 확인하세요.';ctx.toast('Career Start 저장 완료');}
  }
  ...
  function renderResearchStatus(){const at=ctx.getState().meta?.lastResearchExportAt,el=document.getElementById('researchFileStatus');if(el)el.textContent=at?`연구용 파일 생성됨 · ${formatTime(at)} · 활동 원문 제외`:'Jobfit 참여코드를 만든 뒤 언제든 저장할 수 있습니다.'}
...
function renderCentralResearchTask(status,syncedAt,ctx){
  if(!status.enabled)return `<div class="backupTask centralResearchTask disabledTask"><span class="backupTaskN">DB</span><div><b>중앙 연구 DB 제출 <span class="muted">(준비 중)</span></b><p>최종 연구동의 문구와 교수자 접근 설정이 승인되기 전에는 연구자료를 수집하거나 제출하지 않습니다.</p><button class="btn researchBtn" id="centralResearchBtn" disabled>중앙 DB로 제출</button><div class="status" id="centralResearchStatus">현재 자동 전송되지 않습니다.</div></div></div>`;
  return `<div class="backupTask centralResearchTask"><span class="backupTaskN">DB</span><div><b>중앙 연구 DB 제출 <span class="optionalBadge">자발적 선택</span></b><p>제출되는 내용은 인구통계·고용24 점수·진로적응성·강점활용/약점교정 점수·진행률입니다. 활동 원문, AI 대화, 지원서와 면접답변은 제출하지 않습니다. 연구 참여는 언제든 철회할 수 있으며, 철회는 수업·성적에 영향을 주지 않습니다.</p><a class="consentDocLink" href="${ctx.escapeHtml(status.consentDocumentUrl)}" target="_blank" rel="noopener noreferrer">연구 설명문·동의서 확인 ↗</a><div class="researchConsentChecks"><label><input type="checkbox" id="researchReadCheck"><span>연구 설명문과 수집 항목을 확인했습니다.</span></label><label><input type="checkbox" id="researchVoluntaryCheck"><span>참여는 자발적이며 거부해도 수업·성적에 불이익이 없음을 이해했습니다.</span></label><label><input type="checkbox" id="researchAgreeCheck"><span>위 연구목적의 자료 수집·이용에 동의합니다.</span></label></div><button class="btn researchBtn" id="centralResearchBtn">동의하고 중앙 DB로 제출</button>${syncedAt?'<button class="btn outline researchBtn" id="withdrawResearchBtn" type="button">연구 참여 철회·연구자료 삭제</button>':''}<div class="status ${syncedAt?'goodText':''}" id="centralResearchStatus">${syncedAt?`최근 중앙 제출 · ${formatDisplayTime(syncedAt)} · 활동 원문 제외`:`동의하지 않으면 제출하지 않아도 됩니다. 동의 여부는 성적에 영향을 주지 않습니다.`}</div></div></div>`;
}
