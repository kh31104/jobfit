...
export async function render(ctx){
  ...
  root.innerHTML=`<section class="card careerDnaStandard">
    ${styleBlock()}
    <div class="sectionHead"><div><div class="kicker">STEP 1 · STANDARD SET v1</div><h2>Career DNA</h2><p>내가 생각하는 나와 검사에서 나타난 나를 비교해 <b>자기이해 가설</b>을 만듭니다.</p></div><span class="badge">3~4주차 연결</span></div>
    <div class="progress"><span style="width:14%"></span></div>
    <div class="callout info"><b>오늘의 흐름</b> · Balance Game → Career Anchor → 내가 생각하는 강점 → VIA → 직접 비교 → AI 통합분석 → Career DNA v1</div>
    ...
    <div class="hr"></div><div class="block"><div class="moduleHead"><span>05</span><div><h3>내가 생각하는 나 × 검사에서 나타난 나</h3><p>AI보다 먼저 직접 비교합니다. 검사가 ‘실제 나’의 정답은 아닙니다.</p></div></div>
      <div class="compareColumns"><div class="compareCard qualitative"><b>내가 생각하는 나 · 정성</b><div id="qualSummary">${qualSummaryHtml(balanceAnswers,selfStrengths,ctx)}</div></div><div class="compareCard quantitative"><b>검사에서 나타난 나 · 정량</b><div id="quantSummary">${quantSummaryHtml(scale,anchorResponses,bonusItems,viaTop5,ctx)}</div></div></div>
    ...
    <div class="hr"></div><div class="block"><div class="moduleHead"><span>06</span><div><h3>AI 통합분석</h3><p>현재 입력된 결과만 사용해 자기이해 가설을 만드는 프롬프트를 생성합니다.</p></div></div>
    ...
    <div class="hr"></div><div class="block"><div class="moduleHead"><span>07</span><div><h3>Career DNA 가설 v1</h3><p>AI 통합분석 결과를 검토한 뒤 필요한 부분만 저장합니다. 4주차에는 실제 경험으로 이 가설을 확인합니다.</p></div></div>
    ...
    <div class="actions"><button class="btn primary" id="saveDNA">Career DNA 저장</button><button class="btn secondary" id="nextStep">STEP 2 경험·역량으로 이동 →</button></div><div class="status" id="status"></div>
  </section>`;
  ...
  function saveData(showToast){
    const via=readVia(),anchor=scale?scoreAnchor(scale,anchorResponses,bonusItems):{complete:false,scores:{},ranking:[]};
    const data={...saved,
      standard:{version:'career-dna-standard-set-v1',savedAt:new Date().toISOString(),sourceModules:['balance','careerAnchor','selfStrengths','via','comparison','aiIntegration']},
      ...
      promptMeta:{version:PROMPT_VERSION,moduleStatus:moduleStatus(balanceAnswers,anchor,strengthSelection,via)}
    };
    const profile=buildProfile(data);
    ctx.saveState({assessments:{careerDNA:data},artifacts:{careerDNAProfile:profile}});document.getElementById('status').textContent='Career DNA가 이 브라우저에 저장되었습니다.';if(showToast)ctx.toast('Career DNA를 저장했습니다.');return data;
  }
...
function moduleStatus(balance,anchor,strengths,via){return{balance:{status:balance.filter(Boolean).length===7?'complete':balance.filter(Boolean).length?'partial':'none',count:balance.filter(Boolean).length,total:7},careerAnchor:{status:anchor.complete?'complete':Object.values(anchor.scores||{}).some(Boolean)?'partial':'none',count:anchor.complete?40:0,total:40},selfStrengths:{status:strengths.length===5?'complete':strengths.length?'partial':'none',count:strengths.length,total:5},via:{status:via.length===5?'complete':via.length?'partial':'none',count:via.length,total:5}}}
function buildProfile(data){const rank=data.careerAnchor?.ranking||[];return{version:'career-dna-profile-v3',valueClues:(data.balance?.answers||[]).filter(Boolean).map(x=>x.value),careerAnchorTop:rank.slice(0,3),selfStrengths:data.selfStrengths||[],viaTop5:data.viaTop5||[],comparison:data.comparison||{},hypothesis:data.hypothesis||{},updatedAt:new Date().toISOString()}}
function buildPrompt(d,scale){const lines=[];lines.push('당신은 대학생의 자기이해를 돕는 Career DNA 분석 파트너다. 아래 자료를 통합하되, 검사결과를 성격이나 직업의 정답으로 단정하지 않는다. 이번 단계는 인터뷰가 아니라 현재 자료를 한 번에 정리해 자기이해 가설을 만드는 단계다.');lines.push('');lines.push('[분석 원칙]');lines.push('1. 여러 자료에서 반복되는 특징과 서로 다른 결과를 구분한다.');lines.push('2. 학생의 자기인식과 검사결과 중 어느 한쪽을 더 진짜라고 판단하지 않는다.');lines.push('3. 입력되지 않은 내용은 추정하지 않는다.');lines.push('4. VIA 강점명만으로 직무·역량을 단정하지 않는다.');lines.push('5. 직업을 추천하지 않는다. 확인이 필요한 내용은 가설로 남긴다.');lines.push('6. 다음 주 실제 경험으로 확인할 질문을 정확히 3개 만든다.');const bal=(d.balance?.answers||[]).filter(Boolean);if(bal.length){lines.push('','[Balance Game · 내가 생각하는 중요한 가치 단서]');bal.forEach((x,i)=>lines.push(`${i+1}. ${x.label} → ${x.value}`));if(bal.length<7)lines.push(`※ ${bal.length}/7만 응답. 미응답 문항은 해석하지 말 것.`)}const a=d.careerAnchor;if(a?.complete&&a.ranking?.length){lines.push('','[Career Anchor · 검사결과]');a.ranking.forEach(x=>lines.push(`${x.code} ${x.name}: ${x.score}점`));lines.push('※ 점수 차이가 작거나 동점이면 억지로 1·2위를 구분하지 말 것.')}else if(a?.responses?.some(x=>x)){lines.push('','[Career Anchor · 부분응답]','검사가 완성되지 않았으므로 앵커 순위를 해석하지 말 것.')}if(d.selfStrengths?.length){lines.push('','[내가 생각하는 나의 강점]');lines.push(d.selfStrengths.join(' · '));if(d.selfStrengths.length<5)lines.push(`※ ${d.selfStrengths.length}/5만 선택.`)}if(d.viaTop5?.length){lines.push('','[VIA 성격강점]');lines.push(d.viaTop5.join(' · '));if(d.viaTop5.length<5)lines.push(`※ VIA가 ${d.viaTop5.length}/5만 입력됨.`)}const c=d.comparison||{};if(c.repeat||c.connect||c.unexpected||c.verify){lines.push('','[학생이 직접 비교한 결과]');if(c.repeat)lines.push(`반복해서 나타난 부분: ${c.repeat}`);if(c.connect)lines.push(`서로 연결된다고 느끼는 부분: ${c.connect}`);if(c.unexpected)lines.push(`예상과 달랐던 부분: ${c.unexpected}`);if(c.verify)lines.push(`더 확인하고 싶은 부분: ${c.verify}`)}lines.push('','[출력 형식 · Career DNA 가설 v1]','1. 내가 중요하게 생각하는 일의 조건','2. 여러 자료에서 반복되는 강점 후보','3. 현재 강점 후보와 그 근거 상태','4. 자기인식과 검사결과가 일치하거나 연결되는 부분','5. 서로 다르거나 아직 확인이 필요한 부분','6. 4주차 STEP 2 실제 경험으로 확인할 질문 3개','','각 항목은 근거가 된 입력자료를 괄호 안에 짧게 표시한다. 과장하거나 확정적으로 말하지 말고 “~일 가능성”, “~로 보이지만 경험 확인이 필요함”처럼 가설 수준을 유지한다.');return lines.join('\n')}
