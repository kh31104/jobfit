...
function buildProfile(data){const rank=data.careerAnchor?.ranking||[];return{version:'career-dna-profile-v2',valueClues:(data.balance?.answers||[]).filter(Boolean).map(x=>x.value),careerAnchorTop:rank.slice(0,3),selfStrengths:data.selfStrengths||[],viaTop5:data.viaTop5||[],comparison:data.comparison||{},hypothesis:data.hypothesis||{},updatedAt:new Date().toISOString()}}
function buildPrompt(d,scale){const lines=[];lines.push('당신은 대학생의 자기이해를 돕는 Career DNA 분석 파트너다. 아래 자료를 통합하되, 검사결과를 성격이나 직업의 정답으로 단정하지 않는다. 이번 단계는 인터뷰가 아니라 현재 자료를 한 번에 정리해 자기이해 가설을 만드는 단계다.');lines.push('');lines.push('[분석 원칙]');lines.push('1. 여러 자료에서 반복되는 특징과 서로 다른 결과를 구분한다.');lines.push('2. 학생의 자기인식과 검사결과 중 어느 한쪽을 더 진짜라고 판단하지 않는다.');lines.push('3. 입력되지 않은 내용은 추정하지 않는다.');lines.push('4. VIA 강점명이나 Career Anchor 결과만으로 직무·역량을 단정하지 않는다.');lines.push('5. 직업을 추천하지 않는다. 확인이 필요한 내용은 가설로 남긴다.');lines.push('6. 다음 주 실제 경험으로 확인할 질문을 정확히 3개 만든다.');const bal=(d.balance?.answers||[]).filter(Boolean);if(bal.length){lines.push('','[Balance Game · 내가 생각하는 중요한 가치 단서]');bal.forEach((x,i)=>lines.push(`${i+1}. ${x.label} → ${x.value}`));if(bal.length<7)lines.push(`※ ${bal.length}/7만 응답. 미응답 문항은 해석하지 말 것.`)}const a=d.careerAnchor;if(a?.complete&&a.ranking?.length){lines.push('','[Career Anchor · 검사결과]');a.ranking.forEach(x=>lines.push(`${x.code} ${x.name}: ${x.score}점`));lines.push('※ 점수 차이가 작거나 동점이면 억지로 1·2위를 구분하지 말 것.')}else if(a?.responses?.some(x=>x)){lines.push('','[Career Anchor · 부분응답]','검사가 완성되지 않았으므로 앵커 순위를 해석하지 말 것.')}if(d.selfStrengths?.length){lines.push('','[내가 생각하는 나의 강점]');lines.push(d.selfStrengths.join(' · '));if(d.selfStrengths.length<5)lines.push(`※ ${d.selfStrengths.length}/5만 선택.`)}if(d.viaTop5?.length){lines.push('','[VIA 성격강점]');lines.push(d.viaTop5.join(' · '));if(d.viaTop5.length<5)lines.push(`※ VIA가 ${d.viaTop5.length}/5만 입력됨.`)}const c=d.comparison||{};if(c.repeat||c.connect||c.unexpected||c.verify){lines.push('','[학생이 직접 비교한 결과]');if(c.repeat)lines.push(`반복해서 나타난 부분: ${c.repeat}`);if(c.connect)lines.push(`서로 연결된다고 느끼는 부분: ${c.connect}`);if(c.unexpected)lines.push(`예상과 달랐던 부분: ${c.unexpected}`);if(c.verify)lines.push(`더 확인하고 싶은 부분: ${c.verify}`)}lines.push('','[출력 형식 · Career DNA 가설 v1]','1. 내가 중요하게 생각하는 일의 조건','2. 여러 자료에서 반복되는 강점 후보','3. 자기인식에서 먼저 보이는 특징','4. 검사결과에서 새롭게 보인 특징','5. 서로 연결되거나 아직 확인이 필요한 부분','6. 4주차 실제 경험으로 확인할 질문 3개','','각 항목은 근거가 된 입력자료를 괄호 안에 짧게 표시한다. 과장하거나 확정적으로 말하지 말고 “~일 가능성”, “~로 보이지만 경험 확인이 필요함”처럼 가설 수준을 유지한다.');return lines.join('\n')}
