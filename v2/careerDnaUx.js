...
- 반드시 다음 표 형식으로 작성한다.
  강점 키워드 | 입력자료 | 단서 상태 | 4주차 실제 경험에서 확인할 질문
- 상태는 ‘반복 단서’, ‘단일 단서’, ‘경험 확인 필요’ 중 하나만 사용한다.
- 서로 다른 자료 2개 이상에서 같은 방향의 특징이 반복될 때만 ‘반복 단서’로 표시한다.
- 고용24 검사나 VIA 결과 하나만으로 직무역량이라고 표현하지 않는다.
...
function enhanceAiSwot(root){
  ...
  if(hypothesis){
    ...
    const placeholder='AI 결과를 그대로 믿지 말고, 강점·약점 키워드의 단서 상태와 SWOT 근거를 확인한 뒤 맞는 부분·수정할 부분·4주차 경험으로 확인할 부분을 남기세요.';
